...
I/O 多路复用就是通过一种机制，一个进程可以监视多个描述符，一旦某个描述符就绪（一般是读就绪或者写就绪），能够通知程序进行相应的读写操作。但 select，pselect，poll，epoll 本质上都是同步 I/O，因为他们都需要在读写事件就绪后自己负责进行读写，即自己负责将数据从内核空间拷贝到用户空间，这个过程还是阻塞的；而异步 I/O 则无需自己负责进行读写，异步 I/O 的实现会负责把数据从内核拷贝到用户空间。
...
本质上和 select 没有区别，只不过把存储就绪的文件描述符的集合换成了链表，就没有最大连接数限制了。

#### epoll

epoll，全名叫 eventpoll，是 select 和 poll 的增强版本。
...
- epoll 监听的 fd 集合是常驻内核的，它有 3 个系统调用（epoll_create, epoll_ctl, epoll_wait），通过 epoll_wait 可以多次监听这个 fd 集合，只返回准备好那些 fd。
- select 只有一个系统调用，每次都要将监听的 fd 集合从用户态传到内核，有被监听的事件发生时，返回整个集合。
- 也就是说，epoll 返回的是可用的 fd 子集，而 select 返回的是全部 fd，具体哪些可用需要用户遍历判断。
- 但是，**epoll 的性能并不是一定高于 select 的**，当被监听的 fd 集合数量较少且 fd IO 很繁忙时，select 在性能上有优势。
	- 在高并发，但连接活跃度不是很高时，epoll 优于 select，如网站连接；
	- 在并发性不高（被监听的 fd 集合数量较少），但连接很活跃时（fd IO 很繁忙），select 优于 epoll，如游戏连接。
